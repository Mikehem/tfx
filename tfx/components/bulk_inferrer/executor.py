...
class Executor(base_executor.BaseExecutor):
  ...
  def _set_inferred_property(self, output_examples: Optional[types.Artifact],
                             inference_result: Optional[types.Artifact],
                             inferred: int) -> None:
    if inference_result:
      inference_result.set_int_custom_property('inferred', inferred)
    if output_examples:
      output_examples.set_int_custom_property('inferred', inferred)
  ...
  def Do(self, input_dict: Dict[Text, List[types.Artifact]],
    ...
    if 'model_blessing' in input_dict:
      ...
      if not model_utils.is_model_blessed(model_blessing):
        self._set_inferred_property(inference_result, output_examples, 0)
    ...
    self._set_inferred_property(inference_result, output_examples, 1)
  ...
  def _run_model_inference(
    ...
    Args:
      ...
      output_examples: Optional output examples artifact.
      inference_result: Optional inference result artifact.
      ...
      for split in artifact_utils.decode_split_names(example.split_names):
        if data_spec.example_splits:
          if split in data_spec.example_splits:
            example_uris[split] = artifact_utils.get_split_uri([example], split)
        else:
          example_uris[split] = artifact_utils.get_split_uri([example], split)
    ...
    with self._make_beam_pipeline() as pipeline:
      ...
      for split, example_uri in example_uris.items():
        ...
        if output_examples:
          ...
          _ = (
              data
              | 'WriteExample[{}]'.format(split) >> _WriteExample(
